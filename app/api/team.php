...
function invite_user(string $name, string $email, string $role, int $team_id): array
{
    ...
    $html = "
<h2 style=\"font-family:sans-serif;\">You've been invited to WINOJI</h2>
...
if (php_sapi_name() !== 'cli') {
    ...
    match (true) {
        ...
        $method === 'POST' && $action === 'invite' => (function () use ($b, $team_id) {
                require_role('admin');
                $valid_roles = ['admin', 'manager', 'employee'];
                $role = in_array($b['role'] ?? '', $valid_roles) ? $b['role'] : 'employee';
                print json_encode(!empty($b['name']) && !empty($b['email']) ? invite_user($b['name'], $b['email'], $role, $team_id) : ['success' => false, 'error' => 'name and email required']); })(),
        $method === 'POST' && $action === 'remove' => (function () use ($b, $team_id) {
                require_role('admin');
                print json_encode(remove_team_member((int) ($b['user_id'] ?? 0), $team_id)); })(),
        $method === 'POST' && $action === 'update_role' => (function () use ($b) {
                require_role('admin');
                $valid = ['admin', 'manager', 'employee'];
                $role = in_array($b['role'] ?? '', $valid) ? $b['role'] : null;
                print json_encode($role && !empty($b['user_id']) ? update_user_role((int) $b['user_id'], $role) : ['success' => false, 'error' => 'user_id y role requeridos']); })(),
        default => null
    };
    exit;
}
